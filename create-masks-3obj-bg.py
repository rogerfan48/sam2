#!/usr/bin/env python3
"""
SAM2 Video Segmentation Script (3 Objects + Background)
========================================================

This script generates masks for video frames:
- obj1: First object of interest
- obj2: Second object of interest
- obj3: Third object of interest
- bg: Background (automatically generated as everything except obj1, obj2, and obj3)

Total: 3 objects + 1 background = 4 masks
...
2. Run: python create-masks-3obj-bg.py
3. Check the output in VIDEO_DIR/masks/

The script will:
- Load frames from VIDEO_DIR/frames/
- Generate masks for obj1, obj2, and obj3 based on your click points
- Automatically generate background mask (inverse of obj1 + obj2 + obj3)
- Save masks to VIDEO_DIR/masks/obj1/, obj2/, obj3/, bg/
...
OBJ1_FRAME_IDX = 10      # Frame number for obj1 (0 = 00000.jpg)
OBJ2_FRAME_IDX = 10      # Frame number for obj2
OBJ3_FRAME_IDX = 0      # Frame number for obj3
VISUALIZATION_FRAME_IDX = 10  # Frame to use for visualization output
...
OBJ1_LABELS = [1, 1, 1]  # All positive clicks
...
OBJ2_LABELS = [1, 1, 1]  # All positive clicks

# Object 3: Click Points and Labels
OBJ3_POINTS = [
    (250, 230),
    (395, 200),
    (400, 240)
]
OBJ3_LABELS = [1, 1, 1]  # All positive clicks
...
print("=" * 70)
print("SAM2 Video Segmentation (3 Objects + Background)")
print("=" * 70)
...
if not (0 <= OBJ3_FRAME_IDX < max_frame):
    raise ValueError(f"OBJ3_FRAME_IDX ({OBJ3_FRAME_IDX}) out of range [0, {max_frame-1}]")
...
obj3_points = np.array(OBJ3_POINTS, dtype=np.float32)
obj3_labels = np.array(OBJ3_LABELS, dtype=np.int32)
...
if len(obj3_points) != len(obj3_labels):
    raise ValueError("OBJ3_POINTS and OBJ3_LABELS must have same length")
...
print(f"  obj3: {len(obj3_points)} points on frame {OBJ3_FRAME_IDX} ({frame_names[OBJ3_FRAME_IDX]})")
print(f"  bg: Auto-generated (inverse of obj1 + obj2 + obj3)")
...
objects_info = [
    ...
    ("obj3", obj3_points, obj3_labels, 3, OBJ3_FRAME_IDX),
]
...
# Generate background masks (inverse of obj1 + obj2 + obj3)
for frame_idx in video_segments:
    ...
    # Remove areas where obj1, obj2, or obj3 are detected
    for obj_id in [1, 2, 3]:
    ...
    # Add background mask with obj_id = 4
    video_segments[frame_idx][4] = background_mask
...
if VISUALIZATION_FRAME_IDX in video_segments:
    ...
    # Show obj1, obj2, obj3 (skip bg in 2x2 grid, or you can create a larger grid)
    mask_positions = [(0, 1), (1, 0), (1, 1)]
    mask_titles = ["Object 1", "Object 2", "Object 3"]
    display_objects = [("obj1", 1), ("obj2", 2), ("obj3", 3)]

    for idx, ((obj_name, obj_id), title) in enumerate(zip(display_objects, mask_titles)):
    ...
    viz_path = os.path.join(OUTPUT_DIR, "four_masks_visualization.png")
    plt.tight_layout()
    plt.savefig(viz_path, bbox_inches='tight', dpi=150)
    plt.close()

    print(f"✓ Visualization saved to: {viz_path}")
    print(f"  Note: Visualization shows 3 objects (bg not shown in grid)")

print("\n" + "=" * 70)
print("SUMMARY")
print("=" * 70)
print(f"✓ Successfully generated masks (3 objects + background):")
print(f"  - obj1: {OUTPUT_DIR}/obj1/")
print(f"  - obj2: {OUTPUT_DIR}/obj2/")
print(f"  - obj3: {OUTPUT_DIR}/obj3/")
...
print(f"  (everything NOT covered by obj1, obj2, or obj3)")
print("\n" + "=" * 70)
